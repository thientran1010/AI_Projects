...
class test_DQNAgent(unittest.TestCase):
    def setUp(self):
        ...
        self.agent = DQNAgent.DQNAgent(gym.make("LunarLander-v3",render_mode="human"), model_class=r"dqnModel_lunar_lander.pt", hparams = self.params)
    ...
    def test_optimize_output_int(self):
        output = self.agent.optimize_model()
    ...
    def test_plot_loss_curve_invalid_input(self):
        with self.assertRaises(TypeError):
            self.agent.plot_loss_curve(['a'])
